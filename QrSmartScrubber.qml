...
Rectangle {
  ...
  property var customDisplay: null
  ...
  border.width: 1
  border.color: isScrubbing ? Color.accent : (scrubMidMouse.containsMouse ? Util.alpha(Color.accent, 0.4) : Util.alpha(Color.popups.border || Color.border, 0.3))
  ...
  Row {
    ...
    // Step Down [-]
    Rectangle {
      width: Style.space(18); height: Style.space(20); radius: 0
      color: decM.containsMouse ? Util.alpha(Color.accent, 0.2) : "transparent"
      Text { text: "−"; font.pixelSize: Style.space(8); font.bold: true; color: Color.popups.text || Color.text; anchors.centerIn: parent }
      MouseArea {
        id: decM
    ...
    Rectangle {
      id: midBadge
      height: Style.space(20)
      width: Math.max(Style.space(44), valLabel.implicitWidth + Style.space(10))
      ...
      Text {
        ...
        text: {
          if (scrubRoot.customDisplay) return scrubRoot.customDisplay(scrubRoot.value)
          return (scrubRoot.label ? (scrubRoot.label + ": ") : "") + (scrubRoot.integerOnly ? Math.round(scrubRoot.value) : scrubRoot.value.toFixed(1)) + scrubRoot.unit
        }
