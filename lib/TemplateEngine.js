...
function formatDateTime(date, format) {
  ...
  return format
    ...
    .replace(/MM/g, month)
    .replace(/DD/g, day)
    .replace(/dddd/g, DAYS_OF_WEEK[d.getDay()])
    .replace(/ddd/g, DAYS_OF_WEEK_SHORT[d.getDay()])
...
function resolveBuiltin(varName, clipboardText) {
  ...
  if (lower === "weekday_short") {
    return DAYS_OF_WEEK_SHORT[now.getDay()];
  }
  if (lower === "month_name") {
    return MONTHS_FULL[now.getMonth()];
  }
  if (lower === "month_short") {
    return MONTHS_SHORT[now.getMonth()];
  }
  if (lower === "clipboard") {
    return clipboardText !== undefined ? String(clipboardText) : "";
  }
  if (lower === "clipboard:upper") {
    return clipboardText !== undefined ? String(clipboardText).toUpperCase() : "";
  }
  if (lower === "clipboard:lower") {
    return clipboardText !== undefined ? String(clipboardText).toLowerCase() : "";
  }
  if (lower === "clipboard:trim") {
    return clipboardText !== undefined ? String(clipboardText).trim() : "";
  }
  if (lower.indexOf("random:") === 0) {
    var parts = varName.substring(7).trim().split(":");
    if (parts[0] === "hex") {
      var hLen = parseInt(parts[1]) || 8;
      var hRes = "";
      for (var hi = 0; hi < hLen; hi++) {
        hRes += Math.floor(Math.random() * 16).toString(16);
      }
      return hRes;
    }
    var digits = parseInt(parts[0]) || 6;
    var result = "";
...
// Extract only custom (non-builtin) variables that require user input
function getCustomVariables(text) {
  var all = extractVariables(text);
  return all.filter(function(v) { return !v.isBuiltin; });
}

// Check if text has custom variables requiring user input
function hasCustomVariables(text) {
  return getCustomVariables(text).length > 0;
}
