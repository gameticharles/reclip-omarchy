...
function matchDateFilter(capturedAt, dateFilter) {
  ...
  var clipDateStr = String(capturedAt).substring(0, 10)

  if (dateFilter === "today") return clipDateStr === todayStr
  if (dateFilter === "yesterday") {
    var yest = new Date(now.getTime() - 86400000)
    return clipDateStr === yest.toISOString().substring(0, 10)
  }
  if (dateFilter === "7d") return (now.getTime() - date.getTime()) <= 7 * 86400000
  if (dateFilter === "30d") return (now.getTime() - date.getTime()) <= 30 * 86400000
  if (dateFilter === "mtd") return date.getFullYear() === now.getFullYear() && date.getMonth() === now.getMonth()

  // Exact date match (e.g. "2026-09-03")
...
function getClipDateCounts(history) {
  ...
  for (var i = 0; i < values.length; i++) {
    var e = values[i]
    if (e && e.capturedAt) {
      var dStr = String(e.capturedAt).substring(0, 10)
