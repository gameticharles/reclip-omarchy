function normalizeEntry(value) {
  ...
  if (type === "image") {
    ...
    if (!path) return null
    var entry = { type: "image", path: path, mime: String(value.mime || "image/png") }
...
function addEntry(history, entry, limit) {
  ...
  var max = limit === undefined ? 300 : Number(limit)
...
function displayRows(history, query, category, limit) {
  var values = Array.isArray(history) ? history : []
  ...
  if (typeof category === "number") {
    ...
    if (typeof limit === "number") max = limit
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    rows.push({
      ...
      kind: kind,
      colorValue: kind === "color" ? rawText.trim() : "",
      fullText: rawText,
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    addEntry: addEntry, removeEntryAt: removeEntryAt, clearHistory: clearHistory,
    ...
    isUrl: isUrl, isCodeSnippet: isCodeSnippet
  }
}
